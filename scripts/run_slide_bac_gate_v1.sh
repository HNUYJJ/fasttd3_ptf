...
ROOT=artifacts/slide_bac_gate_v1
LOGDIR=logs/train/slide_bac_gate_v1
...
for SEED in ${SEEDS}; do
  if [[ "${STAGE}" == "anchor" ]]; then
    ...
    env $(common_env) SEED="${SEED}" WANDB=0 \
      EXP_NAME="slide_bac_anchor_s${SEED}" \
  ...
  for ARM in ${ARMS:-student stand walk run}; do
    ...
    if [[ "${ARM}" == "student" ]]; then
      ...
      ARM_ENV+=(SOURCE_BANK="configs/source_banks/calibration/h1hand_slide_rbo_${ARM}.yaml"
