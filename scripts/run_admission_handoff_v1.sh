...
CELL="${CELL:?set CELL to powerlift_admission_all_fix or truck_admission_h4_fix}"
SEED="${SEED:-1}"
GPU_ID="${GPU_ID:-2}"
...
EXP_NAME="h1hand_${TASK}_admission_handoff_v1_all_s${SEED}_${STAMP}"
...
mkdir -p "${LOG_DIR}"
{
  ...
  echo "protocol_sha256=$(sha256sum configs/experiments/admission_handoff_v1.yaml | awk '{print $1}')"
  echo "implementation_sha256=$(sha256sum \
    ...
    scripts/run_admission_handoff_v1.sh | sha256sum | awk '{print $1}')"
