"""Stair BAC 判决场裁决（预注册，揭盲前定稿，只读）。

本实验检验的是 Bottleneck-Aligned Coverage 指标的**前瞻预测**，
预测已冻结于 docs/experiments/bottleneck_aligned_coverage_v1_prereg_20260728.md：

    zero-shot return 预测   stand(84.9) > walk(31.8) > run(26.2)
    NET(BAC) 预测           walk(0.4617) ≈ run(0.4573) ≫ stand(0.0046)   （差 100 倍）

承接 slide 的教训（该轮 §3.1）：walk 与 run 的 NET 仅差 0.96%，
指标不具备分辨二者的能力，**walk vs run 的相对顺序不作为本轮的预测内容**。
本轮唯一被预测的是 stand 显著劣于 walk 与 run。裁决规则与 slide 保持逐字一致，
不因这一说明而放宽。
...
EVAL_DIR = REPO / "docs/data/stair_bac_gate_v1/source_free_eval"
OUT = REPO / "docs/data/stair_bac_gate_v1/stair_bac_gate_v1_results.json"
...
# 事前冻结的预测（写死在此，防止事后重述）
PREREG = {
    "NET": {"stand": 0.0046, "walk": 0.4617, "run": 0.4573},
    "zero_shot_return": {"stand": 84.9, "walk": 31.8, "run": 26.2},
...
def treatment_audit(arm: str, seed: int) -> dict:
    ...
    hits = sorted(glob.glob(str(REPO / f"models/*stair_bac_{arm}_s{seed}*_20000.pt")))
    if not hits:
...
def main() -> None:
    ...
    print("Stair BAC 判决场（128-ep 面板, 3 learner seeds, 90% t 区间）\n")
