"""FastTD3 训练入口（interface_transfer_v1 副本）。

来源：`vendor/fast_td3/train.py`，逐字复制。**唯一改动**是 HumanoidBench 的
环境构造改用 `src/interface_vec_env.py` 的 `InterfaceHumanoidBenchEnv`，
以便透传 `--policy-type` 启用官方 goal-conditioned 低层接口。
其余算法逻辑（Actor/Critic/replay/更新）一行未动，保证 flat 与 interface
两臂除动作接口外完全同构。vendor 原文件未被修改。
"""
import sys as _sys
from pathlib import Path as _Path
_ROOT = _Path(__file__).resolve().parents[1]
for _p in (_ROOT / "src", _ROOT / "vendor" / "fast_td3", _ROOT / "vendor" / "humanoid_bench_pkg"):
...
def main():
    ...
    if args.env_name.startswith("h1hand-") or args.env_name.startswith("h1-"):
        from interface_vec_env import InterfaceHumanoidBenchEnv

        env_type = "humanoid_bench"
        # 接口选择走环境变量，避免改动 vendor 的 tyro dataclass 与
        # get_args 的 env_name→Args 映射逻辑。空/flat 即原始 61 维控制。
        _ptype = os.environ.get("INTERFACE_POLICY_TYPE", "").strip() or None
        if _ptype in ("flat", ""):
            _ptype = None
        envs = InterfaceHumanoidBenchEnv(
            args.env_name, args.num_envs, policy_type=_ptype, device=device
        )
        eval_envs = envs
        render_env = InterfaceHumanoidBenchEnv(
            args.env_name, 1, policy_type=_ptype, render_mode="rgb_array", device=device
        )
        print(f"[interface_transfer_v1] policy_type={_ptype or 'flat'} "
              f"num_actions={envs.num_actions}")
