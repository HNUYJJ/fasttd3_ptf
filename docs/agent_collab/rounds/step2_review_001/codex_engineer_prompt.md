...
Generated: `20260607T061734Z`

## Role Instructions

You are Codex acting as the implementation and reconciliation agent. Read the research brief and any reviewer response, then turn accepted critique into concrete repo actions: code checks, diagnostic scripts, run cards, tests, or patches. Keep changes narrow, verify them locally, and separate confirmed facts from hypotheses.

## Task

Critically review the Step-2 z-native transfer failure analysis, with special attention to the revised dilution-by-domination hypothesis, the A2 ablation, and whether learned shared-z transfer is still worth compute for package.
...
- If present, read and reconcile this reviewer response before proposing repo actions: `docs/agent_collab/rounds/step2_review_001/claude_reviewer_response.md`.
- If that file is missing, say the reviewer turn has not run yet.

## Primary Briefing: docs/step2_research_briefing.md
...
## Extra Context: docs/agent_collab/research_tasks.md
...
| --- | --- | --- | --- | --- |
| S2-R1 | claude_reviewer | ready | Critically review the Step-2 failure analysis and rank alternative mechanisms. | `rounds/step2_review_001/claude_reviewer_response.md` |
| S2-C1 | codex_engineer | ready | Convert accepted review findings into concrete code checks, diagnostics, or run cards. | `rounds/step2_review_001/codex_engineer_response.md` |
| S2-A2 | human_pi + codex_engineer | pending | Decide whether to launch frozen reach-E + no-teacher A2 as the decisive channel-isolation ablation. | Run card before GPU launch |

## Candidate Follow-ups

| ID | Priority | Task | Why It Matters |
| --- | --- | --- | --- |
| S2-M1 | high | Reconcile on-policy diagnostics with final A/B/C curves and A2 status. | Separates frozen representation harm from teacher-distillation harm. |
| S2-B1 | high | Inspect z-native teacher gating and option beta statistics during A. | Tests whether bad advice was actually selected/distilled often enough to explain collapse. |
| S2-E1 | medium | Draft a trainable-E warm-start run card using the slice reach teacher. | Low engineering cost check for whether reach-pretrained entity features help when allowed to adapt. |
| S2-P1 | medium | Prototype robot/CLS readout pooling as an isolated encoder change. | Tests the dilution mechanism without redesigning the whole shared-z idea. |
