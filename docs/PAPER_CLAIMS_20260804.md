...
> 定位见 `/goal`：**不可能性刻画（主）+ 最小充分方法（次，由主推出）**。
>
> 状态标记：`READY`=证据完备可写 / `PENDING`=实验进行中 / `WEAK`=证据不足，须降级或删

## 0. 论文的一句话

> 迁移效用不是任务对 `(source, target)` 的属性；因此零成本的迁移性指标不可得，
> 而准入与选源可以用一个**可量化的最小交互代价**直接测出来。

两半互为因果：不可能性给出"为什么必须付代价"，最小测量给出"代价是多少"。
...
| # | 主张 | 证据 | 必须同时写出的边界 | 状态 |
|---|---|---|---|---|
| I-1 | 十二个信号族、跨七个信号空间，全部无法预测延迟学习效用 | `impossibility_characterization_of_transfer_prediction_20260730.md` §2 | **不是穷尽性证明**；主张限于"已检验空间内、只依赖 `(source,target)` 的即时量" | `READY` |
| I-2 | 存在**原理性**反例（不是"没测出来"）：静态规格无法区分 slide 与 stair | slide 与 stair 共用同一份 `ClimbingUpwards.get_reward`、常量逐字节相同，walk 的效用为 **+56.95** vs **+0.19 [−5.35,+5.72]** | 只证明"只读规格的量"不行，不排除条件于 learner 状态的量 | `READY` |
...
| I-5 | 相似性是对称的，迁移效用是非对称的 | sibling gate：slide→stair `+15.40 [+5.72,+25.08]` 3/3 正，stair→stair 反向 `−20.79 [−31.61,−9.97]` 0/3 正 | **两个方向的稳健性不对称，必须分别陈述**：负方向（`−20.79`）稳健——sibling 臂有 2.5–3.3pp 的**剂量优势**却仍然输，剂量无法解释；正方向（`+15.40`）**须打折**——剂量优势与胜出同向，本实验不能排除它贡献了一部分。单一任务对 | `READY` |
...
| # | 主张 | 证据 | 必须同时写出的边界 | 状态 |
|---|---|---|---|---|
| II-1 | 选源可用 `K*=10000` 的短跑测出 | `RACING_VIABLE`，两批独立各 3/3；`K=5000` 不稳健（批1 3/3 → 批2 1/3） | 单 target（hurdle）、单候选集合 | `READY` |
...
| II-11 | crawl 作为准入例证的边界：它已能被 scratch 高质量解决 | `984.9 / 1000 = 98.5%` of theory max（reward 为 `[0,1]` 项相乘 × 1000 步，已读 `get_reward` 全文核实）；`site_screen` 判 `NO_SCAFFOLD` | **双重表述缺一不可**：(a) crawl 不适合展示 final-ceiling improvement；(b) 但源在**早期**（K=10k，9/9 显著负，`−44.06`~`−258.77`）与**终点**（100k，`−151.0`）都有害，故 source mismatch 解释**依然成立**，不得被 headroom 解释替代。"最终无 headroom" 不蕴含 "迁移必然无价值" | `READY` |
...
target     hurdle / slide / crawl（+ door、cabinet 等作为负例或已关闭场地）
           ...
           **三者均接近理论回报上限**：crawl 98.5% / slide 95.1% / hurdle 85.1%
           （reward 为 [0,1] 项相乘 × 1000 步）。故本项目全部结论都是
           **在 FastTD3 已能高质量解决的任务上**得到的，这限定了任何
           迁移方法在此处能声称的上限。出处 `post_transfer_autonomy_site_screen_results_20260806.md`
...
1. 不得称提出了"新的迁移性指标"——十二族证明其不可得，racing 是**测量**；
