> ## HISTORICAL —— 本文标题与部分表述已于 2026-08-06/07 撤回
>
> 保留原貌以存证，**但不得作为当前有效结论引用**。三处已撤回：
>
> | 原表述 | 现状 | 出处 |
> |---|---|---|
> | 标题的"不可能性刻画" | **撤回**。无形式证明，正确表述是"十二类已测代理信号的经验性系统失败" | `PAPER_CLAIMS_20260804.md` §5 第 8 条 |
> | "任何只读静态规格的量都无法区分 slide/stair" | **收窄**为"只读 reward 规格的量"。二者的地形几何、transition dynamics、初始状态分布、MJCF 均不同 | 同上 I-2 |
> | `K*=10000` 作为 minimum sufficient measurement | **改述**为"已测试预算 `{2k,5k,10k}` 中最小的稳健 horizon" | 同上 §5 第 9 条 |
>
> 活跃投稿文件见 `docs/PAPER_DRAFT_20260806.md` / `docs/PAPER_OUTLINE_20260804.md`，
> 二者受 `scripts/analysis/claim_linter.py` 强制检查。
> 本文件**不**受措辞规则约束（历史存证），但受"必须带本标记"约束。
